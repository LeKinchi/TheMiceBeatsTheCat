...
std::vector<std::string> Server::getHistory(){
    return message_history;
}

std::string Server::recvInputFromExistingStr(int fd) {
    int nbytesrecv = recv(fd, input_buffer, INPUT_BUFFER_SIZE, 0);
    if (nbytesrecv <= 0) {
        //problem
        if (0 == nbytesrecv) {
...
uint16_t Server::sendMessage(uint16_t source_fd, char *messageBuffer) {
    return send(source_fd, messageBuffer, strlen(messageBuffer), 0);
}

uint16_t Server::sendMessage(uint16_t source_fd, const char *messageBuffer) {
    return send(source_fd, messageBuffer, strlen(messageBuffer), 0);
}
